...
import abi2 from '@/connectivity/abis/napaToken.json';
import {
  auctionaddress,
  marketPlace,
  napaAddress,
...
export default function SectionOne({
      ...
      const response = await axios.get(
        `https://deep-index.moralis.io/api/v2/nft/${nftAddress}/${nftId}?chain=sepolia&format=decimal&media_items=false`,
        {
          headers: {
            ...
            'X-API-Key': 'xcAMKMWTyi0jlCzU8UfUkv2kEDOextN9EAPpinQfC8eI7xwdMN7IZU845IIRQImn',
          },
        }
      );  
  ...
  // const pay = async () =>{
  //   const auctionContract = new ethers.Contract(auctionaddress, auctionAbi, signer);
  //   const getmakebid = await auctionContract.fetchAllAuctionBids(auctionId);
  //   console.log(getmakebid.toString(),"---------------------------");
  // } 
  
  const nextBid = async () => {
    const auctionContract = new ethers.Contract(auctionaddress, auctionAbi, signer);
    const getmakebid = await auctionContract.nextBidPrice("8");
    setBid(getmakebid.toString());
  };
  
  const makeBid1 = async (auctionId: any, amount: string) => {
    const auctionContract = new ethers.Contract(auctionaddress, auctionAbi, signer);
    const auctionDetails = await auctionContract.auctionDetails("7");
    const napatokenContract = new ethers.Contract(napaAddress, napaAbi, signer);
    const getUsdtContract = new ethers.Contract(usdtAddress, usdtAbi, signer);
    const paymentMode = await auctionDetails.paymentType;
    ...
    try {
      ...
      if (paymentMode === "undefined") {
        console.error("Payment type is undefined");
        return;
      }

      console.log("Payment Type:", paymentMode);

      if (paymentMode === 0) {
        const approveTx = await napatokenContract.approve(auctionaddress, amountInWei.toString());
        const approveTxReceipt = await approveTx.wait();
        console.log(`Napa Token Approval transaction confirmed. Gas used: ${approveTxReceipt.gasUsed.toString()}`);
      } else if (paymentMode === 1) {
        const approveTx = await getUsdtContract.approve(auctionaddress, amountInWei.toString());
        const approveTxReceipt = await approveTx.wait();
        console.log(`USDT Token Approval transaction confirmed. Gas used: ${approveTxReceipt.gasUsed.toString()}`);
      } else {
    ...
    try {
      const result = await auctionContract.makeBid("21", amount);
  ...
  const CancelAuction = async (auctionId: string,auctionData:any) => {
    ...
    try {
      ...
      if (signer.address !== auctionData.creator ) {
        ...
        const result = await auctionContract.cancelAuction("21");
        console.log("CancelAuction transaction hash:", result.hash);
      }
    } catch (error) {
      console.error("CancelAuction error:", error);
        ...
        <div className={styles.CustomGrid}>
          <div className={styles.ScOneLeftCont}>
           ...
           onClick={() => { withdrawBid (auctionId,auctionData)} } 
            ...
            {/* <div onClick={pay} className={styles.BottomAction}>
            <button type="button" >
                 Bid
              </button>
          </div> */}  
          <div  onClick={nextBid} className={styles.BottomAction}>
            <button>Get Next Bid</button>
            <p>Current Bid: {bid}</p>
          </div>
            {loading ? (
              <Loading />
