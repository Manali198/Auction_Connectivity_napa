...
export default function MyNFTs(props: any) {
  ...
  const loadNFTs: () => Promise<object[]> = async () => {
    ...
    try {
      ...
      // @ts-ignore
      const res = await getAllNFTsOfUser( metaMaskAccount ? metaMaskNetwork : walletNetwork, (metaMaskAccount || napaWalletAccount?.subAcWalletAddress))
      // @ts-ignore
      console.log( res?.data?.data?.tokenData?.response?.result, "otherNFTs");
      await Promise.all(
              // @ts-ignore
